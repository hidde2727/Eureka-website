...
// + ======================================================================== +
export async function GetAllSuggestionWithVotes(userID, limit=20, offset=0, history=false, search='') {
    return await ExecutePreparedStatement(`
    SELECT * FROM (
        SELECT
        ...
        ${history&&search!=undefined?'WHERE ':''}
        ${!history&&search!=undefined?'AND ':''}
        ${search!=undefined?'i.name LIKE ?':''}
        ...
        ${history&&search!=undefined?'WHERE ':''}
        ${!history&&search!=undefined?'AND ':''}
        ${search!=undefined?'p.name LIKE ?':''}
    ) AS suggestions
    ORDER BY uuid ASC
    LIMIT ? OFFSET ?
    `, [userID, ...(search!=undefined ? [search.concat('%')] : []), userID, ...(search!=undefined ? [search.concat('%')] : []), limit, offset]);
}
export async function GetAmountSuggestionWithVotes(userID, history=false, search=undefined) {
    return (await ExecutePreparedStatement(`
    SELECT COUNT(*) AS count
    FROM (
        SELECT
    ...
    `, [userID, ...(search!=undefined ? [search.concat('%')] : []), userID, ...(search!=undefined ? [search.concat('%')] : []) ]))[0]['count'];
}
